...
def generate_dehash_arguments(log_dict):
    ...
    args = {
        ...
        'bold': -1,
        'print_core': False,
        'dict_path': os.environ.get('PBL_CONSOLE_DICT_PATH', log_dict)
    }
...
def main():
    ...
    parser.add_argument('-t', '--tty', help='serial port (defaults to auto-detect)', metavar='TTY',
                        default=None)
    parser.add_argument('-d', '--log-dict', help='Logging dictionary', default=None)
    ...
    dehasher = None
    if args.log_dict:
        dehasher = LogDehash(**generate_dehash_arguments(args.log_dict))
